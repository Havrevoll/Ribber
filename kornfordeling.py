...
PSD = np.array([[0.01,  0.00], #Det er ukjend kor 0 % er, altså kva som er minste storleik. Må anta at det er ganske lite.
                [0.149, 0.02],	
                [0.297, 0.095],	
                [0.594, 0.28],	
                [1.188, 0.55],	
...
tal_per_g = np.diff(PSD_full[:,1])/(pi/6 * rho * PSD_full[1:,0])
PSD_full[:,2] = 
...
ax.semilogx(PSD_full[0], PSD_full[1], color="red", label="original")
# ax.set_xticks(bins)
