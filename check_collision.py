...
def check_all_collisions(particle, data, ribs):
    ...
    # for rib in [ribs['']]:
    collision_info = check_collision(particle, data, ribs)
    collision_info['rib'] = ribs
...
def check_collision(particle, data, rib):
    ...
    normal,punkt = rib.normal(data[:2])
    ...
    # if (not inside):            
        ...
        if (projection > particle.radius):
                return {'is_collision':False, 'collision_depth': particle.radius - projection.item(0), 'is_resting_contact':False, 'is_leaving':False} #, 'rib':rib}
        ...
        collision_info = dict(collision_depth=particle.radius - projection.item(0), rib_normal=normal, particle_collision_point = position + radiusVec, inside = False)
        ...
        radiusVec = normal * particle.radius

        return dict(is_collision = True, is_resting_contact = False, is_leaving = False, rib = rib, collision_depth = particle.radius - projection.item(0), rib_normal = normal, particle_collision_point = position - radiusVec, inside = True)
    ...
    if (abs(v_rel) < vel_limit and round(np.dot(n, normal[:,0]),3)==1.0 ):
